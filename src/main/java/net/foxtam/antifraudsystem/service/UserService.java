...
public class UserService {
    ...
    public void register(User user) throws UsernameAlreadyExistsException {
        ...
        if (userRepository.existsByUsernameIgnoreCase(user.getUsername())) {
            throw new UsernameAlreadyExistsException(user.getUsername() + " already exists");
        }
    ...
    public User changeRoleByUsername(@NonNull String username, @NonNull Role role) throws WrongRoleException, NotFoundUserException, RoleAlreadyProvidedException {
        if (role == Role.ADMINISTRATOR) throw new WrongRoleException(role.toString());
        Optional<User> userOptional = userRepository.findByUsernameIgnoreCase(username);
        if (userOptional.isEmpty()) throw new NotFoundUserException(username);
    ...
    public void changeLockStatus(@NonNull String username, @NonNull Lock lock) throws NotFoundUserException, WrongRoleException {
        Optional<User> userOptional = userRepository.findByUsernameIgnoreCase(username);
        if (userOptional.isEmpty()) throw new NotFoundUserException(username);
